...
| ② | 特性设计文档 | `references/design-docs/` | 功能规格、行为定义 | 直接引用设计描述 |
| ③ | Oracle知识库 | `references/oracle-kb/` | 高质量参考内容 | 改写为YashanDB版本 |
| ④ | 测试用例 | `references/test-cases/` | 可执行验证SQL | 直接作为示例 |
| ⑤ | 源码 | `references/source/` | 实现细节、行为兜底 | 提取关键逻辑描述 |
...
> ⚠️ **未配置 MCP 时的降级策略**：如果 YashanDB 知识库 MCP 不可用，系统将自动降级到本地 `references/` 目录中的静态资料，按优先级 ②→③→④→⑤ 依次引用。但生成的文档需在末尾标注：`> ⚠️ 本文档生成时 YashanDB 知识库 MCP 不可用，内容基于本地静态资料生成，建议人工复核最新官方文档。`
...
| 知识点 | 设计文档路径 | 说明 |
|--------|-------------|------|
| [待补充] | `references/design-docs/[待补充]` | [待补充] |

### 5.3 Oracle知识库映射

| YashanDB知识点 | 对应Oracle文档 | 映射说明 |
|---------------|---------------|---------|
| 集群基础概念 | `references/oracle-kb/01-Oracle集群基础概念与架构.md` | 改写为YashanDB版本 |
| 集群核心组件 | `references/oracle-kb/02-集群核心组件与内部机制.md` | 改写为YashanDB版本 |
| 资源管理运维 | `references/oracle-kb/03-集群资源管理与日常运维.md` | 改写为YashanDB版本 |
| 性能监控诊断 | `references/oracle-kb/04-集群性能监控与诊断.md` | 改写为YashanDB版本 |
| 性能调优 | `references/oracle-kb/05-集群性能调优实战.md` | 改写为YashanDB版本 |
| 高可用容灾 | `references/oracle-kb/06-高可用与容灾架构.md` | 改写为YashanDB版本 |
| 内核机制 | `references/oracle-kb/07-集群内核机制深度解析.md` | 改写为YashanDB版本 |

### 5.4 测试用例索引

| 知识点 | 测试用例路径 | 说明 |
|--------|-------------|------|
| [待补充] | `references/test-cases/[待补充]` | [待补充] |

### 5.5 源码索引

| 知识点 | 源码路径 | 说明 |
|--------|---------|------|
| [待补充] | `references/source/[待补充]` | [待补充] |
...
2. `references/` 目录下各资料子目录是否存在
3. Oracle 知识库文档是否完整
4. 缺失的资料类型给出补充提示

详见 `scripts/pre-check-references.sh` 和 `README.md` 中的使用说明。
