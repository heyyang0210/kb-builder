...
VALID_MANIFEST = Path("contracts/enterprise-profile/v1/fixtures/valid/minimal-yashandb.json")
...
class PlatformProfileLoaderTest(unittest.TestCase):
    ...
    def test_missing_resource_is_reported_without_absolute_path(self):
        with tempfile.TemporaryDirectory(prefix="kpg-profile-") as directory:
            root = Path(directory)
            schema_target = root / "contracts/enterprise-profile/v1/enterprise-profile.schema.json"
            schema_target.parent.mkdir(parents=True)
            schema_target.write_bytes((REPOSITORY_ROOT / "contracts/enterprise-profile/v1/enterprise-profile.schema.json").read_bytes())
            manifest = root / "profiles/yashandb.json"
            manifest.parent.mkdir(parents=True)
            manifest.write_bytes((REPOSITORY_ROOT / VALID_MANIFEST).read_bytes())
            with self.assertRaises(ProfileError) as raised:
                load_profile(repository_root=root, registry={"yashandb": "profiles/yashandb.json"}, env={})
    ...
    def test_symlink_escape_is_rejected(self):
        with tempfile.TemporaryDirectory(prefix="kpg-profile-") as directory, tempfile.TemporaryDirectory(prefix="kpg-outside-") as outside:
            root = Path(directory)
            schema = root / "contracts/enterprise-profile/v1/enterprise-profile.schema.json"
            schema.parent.mkdir(parents=True)
            schema.write_bytes((REPOSITORY_ROOT / "contracts/enterprise-profile/v1/enterprise-profile.schema.json").read_bytes())
            target = Path(outside) / "domain.json"
            target.write_text("{}", encoding="utf-8")
            resource = root / "contracts/enterprise-profile/v1/fixtures/resources/domain.json"
