...
```bash
cd agent-runner
python3 ../scripts/pingcode-pipeline.py
```
...
config/pingcode.json
  ...
  IndexBuilder.save_json()  ← 生成索引
       ↓
  scripts/pingcode/data/{space_key}/
```
...
# 爬取空间
output_dir = Path('scripts/pingcode/data')
...
然后运行：
```bash
python3 scripts/pingcode/cli/space_crawl.py --space YASDOC
```
...
```bash
python3 scripts/pingcode/cli/space_crawl.py --space YASSTORAGE --workers 10 --max-pages 500
```
...
这会将 `refs/pingcode/YASSTORAGE_内幕文档/` 整合到 `scripts/pingcode/data/YASSTORAGE/`。
