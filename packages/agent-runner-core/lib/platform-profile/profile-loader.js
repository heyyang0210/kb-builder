...
const ALLOWED_RESOURCE_PREFIXES = [
  'packages/platform-contracts/enterprise-profile/',
  'knowledge/domain/',
  'knowledge/skills/',
  'packages/agent-runner-core/lib/agents/',
  'tools/knowledge-processing/pingcode-processing/skills/',
...
function loadProfile(options = {}) {
  ...
  const schemaPath = path.join(repositoryRoot, 'packages/platform-contracts/enterprise-profile/v1/enterprise-profile.schema.json');
  let schema;
  try {
