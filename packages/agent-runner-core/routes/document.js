...
function loadDocPathsConfig() {
  ...
  return [
    { id: 'output', name: '生成文档', path: path.join(AGENT_RUNNER_RUNTIME_ROOT, 'output'), writable: true, description: '知识库生成器产出的文档' }
  ];
}
...
const COMMENTS_STORE_PATH = process.env.DOCUMENT_COMMENTS_PATH
  ...
  : path.join(AGENT_RUNNER_RUNTIME_ROOT, 'data', 'document-comments.json');
...
// 文档上传预处理配置
// ============================================
const DOC_UPLOAD_DIR = path.join(AGENT_RUNNER_RUNTIME_ROOT, 'tmp', 'doc-uploads');
const DOC_PROCESSED_DIR = path.join(AGENT_RUNNER_RUNTIME_ROOT, 'tmp', 'doc-processed');
