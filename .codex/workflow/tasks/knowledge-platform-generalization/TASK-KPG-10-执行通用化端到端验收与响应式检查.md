...
- [x] 默认 3500/4100/8001 已重启到当前版本；Socket.IO/SSE 真实协议验证通过。真实外部平台和生产黄金样例另需授权环境。
...
- 后续复测：确认旧进程 `activeTasks=0` 后受控重启默认 3500/4100/8001；7 个关键 URL 均返回 200，Node/Python 指纹一致，聚合为 `ok`，端口持续监听。
- Socket.IO：真实客户端连接、`subscribe`/`unsubscribe` 回调确认通过；Python SSE 四条路径均通过真实 TestClient 流式响应头与事件内容验证。

残余问题：真实 PingCode/MCP 连接、外部模型完整生成和生产数据黄金样例未验证；这些需要人类提供授权环境。Python 开发默认 8000 与部署 8001 的文档口径仍需后续统一。机器可读兼容清单已由真实协议测试补强。
