...
| TASK-KPG-10 | 执行通用化端到端验收与响应式检查 | 聚焦验收通过，待部署验证 | 隔离 API/聚合、回归、构建和 Playwright 通过；默认端口与真实流待验 |
| TASK-KPG-11 | 完成通用化文档对账与改造收口 | 有条件收口 | 文档已对账；合并/部署受 TASK-KPG-10 残余门禁约束 |
...
| 项目 | 目标 | 当前状态 |
|---|---|---|
| `/knowledge-center/` | 统一平台入口 | 隔离当前版本已实现；默认 3500 旧进程待重启 |
| `/prompt-generator.html` | 保持可用并提示迁移 | 当前可用，未增加提示 |
| `/pingcode-materials/` | 保持可用并提示迁移 | 当前可用，未增加提示 |
| 既有 API | 本轮保持兼容 | 聚焦 HTTP/API 代理通过；Socket.IO/SSE 真实流待部署验证 |
