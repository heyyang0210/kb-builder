...
> 更新日期：2026-08-27
> 总体状态：阶段 A-E 已完成；真实外部连接、模型黄金样例和凭证治理待人类决策
> 状态单一入口：本文
...
| TASK-KPG-08 | 建设知识中心建设平台统一前端入口 | 已完成 | 统一外壳、双工作区入口、降级恢复和 Playwright 四视口通过 |
| TASK-KPG-09 | 保留旧入口并增加平台迁移提示 | 已完成 | 两个旧入口迁移提示、深链参数和 API 代理兼容测试通过 |
| TASK-KPG-10 | 执行通用化端到端验收与响应式检查 | 已完成（外部环境项除外） | 默认端口、真实协议、隔离 API/聚合、回归、构建和 Playwright 通过 |
| TASK-KPG-11 | 完成通用化文档对账与改造收口 | 已完成（外部环境项除外） | 文档已对账；外部连接、模型和凭证治理交由人类决策 |

## 3. 配置迁移矩阵

| 类别 | 当前事实源 | 目标归属 | 状态 |
|---|---|---|---|
| 品牌与平台名称 | 前端与包信息中的直接文案 | 企业能力包品牌段 | 文档生成、资料加工和统一入口均已读取 |
| 领域模型 | `domain/yashandb/` 和加工规则 | 企业能力包引用 | 已引用事实源并由资料加工运行上下文读取 |
...
| 项目 | 目标 | 当前状态 |
|---|---|---|
| `/knowledge-center/` | 统一平台入口 | 默认 3500 当前版本已运行并返回 200 |
| `/prompt-generator.html` | 保持可用并提示迁移 | 当前可用，未增加提示 |
| `/pingcode-materials/` | 保持可用并提示迁移 | 当前可用，未增加提示 |
| 既有 API | 本轮保持兼容 | HTTP/API 代理、Socket.IO 订阅/退订和四条 SSE 真实协议通过 |
| Node/Python 契约一致性 | 相同配置指纹和脱敏上下文 | 已通过；完整能力包 29 个资源指纹一致 |
| Playwright 四视口 | 无溢出、遮挡和控制台错误 | 隔离 Chromium 四视口通过 |
...
### 6.8 TASK-KPG-08 统一前端入口

- 新增 `agent-runner/frontend/knowledge-center/` 静态平台外壳和 3500 路由，首屏读取 `/knowledge-center/api/platform/context`，展示企业身份、配置一致性、连接器摘要及两个专业工作区。
- 资料加工与文档生成保持独立深链；故障模块禁用入口并提供刷新恢复，双模块不可用展示可恢复错误。
- 采用中文、中高密度、系统无衬线、6px 圆角、低动效和移动端单列布局；未引入第三方运行依赖。
- 隔离真实路由返回 200；Playwright Chromium 四视口及单模块降级测试 5/5 通过，无页面级横向溢出。
- TASK-KPG-09 继续负责旧入口迁移提示，TASK-KPG-10 负责完整兼容回归和截图归档。

### 6.9 TASK-KPG-09 旧入口迁移提示与兼容

- `/prompt-generator.html` 和 `/pingcode-materials/` 均增加可关闭的统一平台迁移提示，不改变当前 URL、查询参数、批次上下文或原业务操作。
- 既有复制提示浮层补充 `pointer-events: none`，避免遮挡提示按钮和其他控件。
- Python 前端构建通过；Playwright Chromium 旧入口、查询参数/深链和 `/pingcode-api/api/health` 代理测试 3/3 通过。
- 未新增重定向或 API 响应字段；旧 API、Socket.IO、SSE 全量兼容和四视口回归留待 TASK-KPG-10。

### 6.10 TASK-KPG-10 通用化端到端验收与响应式检查

- Node 聚焦 Jest 31/31、Python 聚焦 unittest 11/11、资料加工前端构建通过。
- 隔离 Node 14110、Python 18010、3500 13510 真实调用：双端平台上下文均 200，配置指纹一致；聚合为 200/ok；旧入口均 200。
- Playwright Chromium 统一入口四视口、单模块降级、旧入口深链和 PingCode API 代理 8/8，通过无横向溢出和测试控制台错误。
- 安全扫描覆盖运行源码、统一入口和聚合响应，未发现 token/password/secret 或服务器绝对路径；聚合异常与超时测试通过。
- 环境边界：默认 4100/8001/3500 已在确认无活动任务后重启到当前版本；真实 PingCode/MCP、外部模型完整生成和生产黄金样例仍未验证。
- 已知非阻塞警告：Vite 既有大 chunk；Python 8000/8001 端口漂移留待后续部署确认。

### 6.11 TASK-KPG-11 文档对账与改造收口

- 对账完成：PLAN、PROGRESS、专项 README、任务索引、12 张任务卡、平台架构/运行上下文设计、Schema 说明和 `prompt.md` 状态一致。
- 本专项提交按任务与补验独立保留：`6cf4eb1`、`5e067c0`、`ca3b31a`、`0ad3742`、`66ff273`、`66a3f1f`、`25089c9`、`bd8fb42`、`2b4a2bb`、`9e413e9`、`cacf617`、`e3527e6`、`d042a1f`、`35caf7b` 及最终对账提交。
- 回退：按任务提交粒度恢复代码和文档，选择上一能力包版本并重启服务；不执行数据反向迁移，保留旧入口/API。
- 合并建议：具备合并到 `dev` 的代码和文档条件；合并前需人类确认明文凭证、端口部署、绝对路径、安全债务和真实外部环境风险。

### 6.12 阶段 E 补充验收

- 默认部署：确认旧 Python `activeTasks=0` 后受控重启 3500/4100/8001，当前版本 7 个关键 URL 均返回 200，双端配置指纹一致，聚合为 `ok`，端口持续监听。
- Socket.IO：增加真实客户端依赖和测试，连接、`subscribe`、`unsubscribe` 及 `task_id` 确认回调通过；旧客户端不传回调时行为保持兼容。

### 6.13 知识中心融合与隔离验证补充

- 统一网关新增只读事实投影 `/knowledge-center/api/platform/overview`，聚合文档统计、工作台、资料批次、知识库和索引摘要；携带 `asOf`、分区状态和降级错误，不返回正文、凭证或服务器路径。
- 新增 `restart-knowledge-center-isolated.sh`，默认使用 13510/14110/18010，通过 PID、工作目录和命令行精确管理本分支进程，连续重启及三端健康检查通过，不触碰 dev 服务。
- 知识中心前端改为统一投影优先，补齐总览、手册生产进展泳道、来源预览与快照固化、知识加工、文档生产链、只读文档管理和独立策略管理视图；投影不可用时保留明确的评审示例标识。
- Playwright Chromium 实测 1440x900、1024x768、768x1024、375x812：无页面级横向溢出、生产进展和来源预览可达、控制台无新增错误。移动端通过打开导航后完成视图切换。
- 前端性能优先项完成首轮拆分：Markdown 编辑器、图谱、质量工作台改为异步视图，Vite 按编辑器/Mermaid/ECharts/Cytoscape 等域拆包；业务页最大包由约 3.3 MB 降至约 46 KB，保留 500 kB 告警用于后续性能基线治理。
- 限制：GitLab 仍为受控模拟；CAS、外部模型、真实回写、文档完整度和知识点覆盖度的服务端口径不在本轮前端自行计算。
- 后续前端归并：运行入口按 Demo 评审骨架继续收敛为八个任务型导航；大纲管理、模板管理、资料清洗独立，文档生产承载来源选择与生成策略。生成策略的参考文档分类、优先级及 Agent/Skill/Prompt 编排已在页面框架中明确，缺少事实源的实例显示“待创建”。
- 侧栏交互：统一入口已支持展开、64px 图标态、完全隐藏及 200～360px 调宽，并通过本地存储恢复桌面设置；移动端保持抽屉导航。
- SSE：四条 Python 流式路由均验证 `text/event-stream`、缓存/缓冲响应头和事件内容。
- 兼容基线：机器清单从纯实现对账升级为真实 Socket.IO/SSE 协议证据；兼容 Jest 5/5、Python SSE 4/4。
- 外部环境项：真实 PingCode/MCP、外部模型完整生成和生产黄金样例未执行，需要人类提供授权环境；明文凭证轮换和历史清理仍需人工决策。

- 任务开始、完成、阻塞、范围变化或产生提交时更新本文。
- 只有验收证据齐全才能将任务标记为“已完成”。
- 每项完成记录实际变更、测试命令与结果、提交哈希、残余问题和下一任务输入。
- 任务卡保存局部执行日志；跨任务累计状态、迁移矩阵和风险只在本文维护。
#### 补充记录：文档生成策略资源演进与技术债登记

- 时间：2026-08-28 11:20:00
- 设计：`enterprise-profile/v1` 增加可选 `generationPolicies`，按稳定文档类型和 `full`/`incremental`/`outline-extraction` 模式声明来源确认、Agent、Skill、Prompt、模板、质量门禁和逻辑输出目录。
- 实现：YashanDB 能力包登记七类策略；加载器校验资源引用、人工确认和类型/模式唯一映射；Prompt Generator 优先按策略选择并返回策略 ID、版本和指纹，旧字段继续保留。
- TODO：新增 `.codex/workflow/todos/知识中心通用化待处理风险与技术债.md`，记录绝对路径、明文凭证、旧前端暴露、固定盐/默认密钥和 Vue 大 chunk；本轮按要求不修改这些既有内容。
- 风险：文档类型 ID、全量候选来源回退、输出目录映射和质量门禁强度仍需人工确认。

### 6.14 知识中心前端模块化与响应式收敛

- 多角色完成架构冲突、前端边界和 Playwright 独立审计；九项冲突已由用户批准 B 方案并进入 `07-决策与风险/知识中心-前端-改造决策-决策记录文档.md`。
- 前端拆分为八个独立模块的 ES Module，公共 API、状态、UI 组件和 DOM 工具分别进入 `common`；资料清洗不再通过字符串替换复用其他页面。
- 移除生成策略浏览器硬编码、不可达旧页面和本地伪预览；未接入操作使用禁用态，不写入演示业务事实。
- `900px` 及以下改用抽屉导航，补齐 `inert`、焦点闭环、`aria-expanded`、44px 触控区和容器宽度折行。
- 网关聚合不再将缺失数字改写为零；内部 `dataset_*` 标识不再进入用户首屏。
- 决策落地：用户批准 `DEC-KC-FE-001` 至 `DEC-KC-FE-009` 的 B 方案；八模块采用 `/knowledge-center/<module>` 正式路由，支持直接访问、刷新恢复和浏览器前进/后退。
- 验收：真实 context、overview、根入口和 `/knowledge-center/assets` 均返回 `200`；Jest 18/18，Chromium 8/8；1440 和 375 最终截图无溢出、遮挡或控制台错误。
- 未完成边界：权限导航、真实空间/用户上下文、手册进度事实、已发布生成策略 API 和 GitLab 真实能力仍需后端契约或专项验收；完整业务页继续受模块详细设计门槛约束。
## 2026-08-31 第一、二阶段前端改造

- 已完成负责人工作台重构：真实任务摘要、资产概况、风险和生产进度按决策顺序组织。
- 已移除负责人首屏的统一投影、事实源和内部知识库版本 ID 等研发信息。
- 已建立 `common` 视觉基础和 `modules/dashboard` 工作台目录，接入固定版本 Lucide 图标及许可证。
- 已将风险和生产进度的未建设状态改为紧凑、可下钻的能力状态，不使用演示业务数据。
- 第三阶段暂停：八个业务模块需分别补齐事实源、状态、DTO、权限、错误和页面流转详细设计后实施。
