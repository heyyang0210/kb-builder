...
async function seedUnderReview(api) {
  ...
  const baselines = await call(api, 'GET', '/knowledge-center/api/incremental-tasks/baselines?handbookId=DB-001&businessVersion=23.4');
  const baselineVersionId = baselines.find(item => item.current)?.id;
...
test.describe('审核与发布多角色 E2E', () => {
  ...
  test('阅读、差异、行级意见、审核和发布均由真实后端驱动', async ({ page, request }) => {
    ...
    await expect(page.locator('[data-review-anchor-status]')).toContainText('已定位第 1 行');
    await page.locator('[data-review-comment-form] textarea').fill('请核对 23.4 参数依赖');
    await page.locator('[data-review-comment-form]').getByRole('button', { name: '提交意见' }).click();
    await page.getByRole('button', { name: '3 处理审核意见' }).click();
