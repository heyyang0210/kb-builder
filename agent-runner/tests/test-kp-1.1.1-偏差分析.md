...
**设计文档要求**：
- 优先级 2：特性设计文档检索（`references/design-docs/`）
- 优先级 3：Oracle 知识库检索（`references/oracle-kb/`）
- 优先级 4：测试用例检索（`references/test-cases/`）
- 优先级 5：源码检索（`references/source/`）
...
**原因分析**：
1. **basePath 错误**：`RetrievalStrategy` 的 basePath 是 `agent-runner/`，但 `references/` 目录在 `agent-runner` 的上级目录
...
2. **修复 Oracle 索引解析**：
   - 检查 `references/oracle-kb/README.md` 的实际格式
