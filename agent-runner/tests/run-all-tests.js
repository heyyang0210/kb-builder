 ...
 * 
 * 用法：
 *   node tests/run-all-tests.js                    # 运行全部测试（按顺序）
 *   node tests/run-all-tests.js unit               # 仅运行单元测试
 *   node tests/run-all-tests.js integration        # 仅运行集成测试
 *   node tests/run-all-tests.js preprocessing      # 仅运行预处理测试
 *   node tests/run-all-tests.js e2e                # 仅运行 E2E 测试
 *   node tests/run-all-tests.js functional         # 仅运行功能测试
 *   node tests/run-all-tests.js --verbose          # 详细输出
 *   node tests/run-all-tests.js --coverage         # 带覆盖率
 *   node tests/run-all-tests.js --report           # 生成测试报告
 *   node tests/run-all-tests.js --skip-e2e         # 跳过 E2E 测试
 *   node tests/run-all-tests.js --parallel         # 并行执行（各分类间）
 */
...
const TEST_CATEGORIES = {
  unit: {
    ...
    files: [
      ...
      'tests/step-executor.test.js',
      'tests/tools.test.js',
      'tests/workflow-engine.test.js',
      'tests/yaml-metadata-processor.test.js',
      'tests/retrieval-query-builder.test.js',
    ]
  },
  preprocessing: {
    name: '预处理模块测试',
  ...
  functional: {
    ...
    files: [
      'tests/button-functional-test.js',
      'tests/test-kp-1.1.1.js',
      'tests/test-yaml-format.js',
      'tests/test-yaml-format-fix.js',
      'tests/system-test-datatype-mapping.js',
    ]
  },
  e2e: {
    name: 'E2E 测试',
    ...
    files: [
      ...
      'tests/e2e/02-tab-navigation.spec.js',
      'tests/e2e/03-sidebar.spec.js',
      'tests/e2e/04-doc-generation.spec.js',
      'tests/e2e/05-config-modal.spec.js',
      'tests/e2e/06-doc-management.spec.js',
      'tests/e2e/07-api-integration.spec.js',
      'tests/e2e/11-workflow-execution.spec.js',
      'tests/e2e/13-retrieval-optimization.spec.js',
    ]
  }
};
