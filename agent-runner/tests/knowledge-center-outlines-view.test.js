...
describe('知识中心大纲管理前端契约', () => {
  const view = () => read('frontend/knowledge-center/modules/outlines/view.js');
  ...
  test('上传文件由管理员创建候选并由服务端统一解析', () => {
    ...
    const uploader = read('frontend/js/components/OutlineUploader.js');
  ...
  test('详情深链、无效手册与阶段参数有明确契约', () => {
    const navigation = read('frontend/knowledge-center/common/state/navigation.js');
    const api = read('frontend/knowledge-center/common/api/platform-api.js');
    const app = read('frontend/knowledge-center/app.js');
