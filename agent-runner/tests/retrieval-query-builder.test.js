const {
  ...
  splitQueryList,
  getSynonymGroups
} = require('../lib/retrieval-query-builder');
const { extractDirectReferences, extractKeywordsFromKnowledgePoint, smartSplit } = require('../lib/direct-generate/prompt-parser');
const PlannerAgent = require('../lib/agents/planner-agent');

// Mock LLM client to return predictable synonym expansions
jest.mock('../lib/synonym-expander', () => {
...
`;
  ...
  test('extractDirectReferences 复用查询词构建并保留参考文件', async () => {
    ...
    expect(result.referenceFiles).toContain('templates/base.md');
    expect(result.referenceFiles).toContain('templates/default.md');
