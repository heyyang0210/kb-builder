...
describe('知识资产工作台前端契约', () => {
  const view = () => read('frontend/knowledge-center/modules/assets/view.js');
  ...
  test('管理员操作由角色能力控制', () => {
    ...
    const auth = read('frontend/knowledge-center/common/state/auth-state.js');
    expect(auth).toContain('PLATFORM_ADMIN');
  });

  test('使用服务端搜索分页和专业模块入口', () => {
    const api = read('frontend/knowledge-center/common/api/platform-api.js');
  ...
  test('操作入口同级展示，关闭使用 Lucide 图标', () => {
    ...
    const css = read('frontend/knowledge-center/styles.css');
  ...
  test('基础字号令牌接近浏览器 110% 观感', () => {
    const tokens = read('frontend/knowledge-center/common/tokens.css');
    const base = read('frontend/knowledge-center/common/base.css');
