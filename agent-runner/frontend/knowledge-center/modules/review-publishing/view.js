...
function reader(projection, task, ui = {}) {
  ...
  const documentId = matchingOperation.length === 1
    ...
    : (task?.target?.scope === 'document' && scopedDocuments.length === 1
      ? scopedDocuments[0]
      : '');
...
function opinions(task, review) {
  ...
  const records = comments.length ? comments.map(comment => {
    ...
    const location = comment.anchor ? `<button class="button link" type="button" data-review-comment-locate="${esc(comment.id)}" data-document-id="${esc(comment.anchor.documentId || '')}" data-review-line="${esc(comment.anchor.lineRange?.startLine ?? comment.anchor.lineRange?.start ?? '')}">查看修改位置</button>` : '';
...
function detail(task, tasks, allTasks, step, projection, diffData, ui) {
  ...
  const drawerComments = list(review?.comments).filter(comment => {
    const anchorPath = comment.anchor?.documentId || comment.anchor?.path || '';
    return !anchorPath || anchorPath === (projection?.file?.path || '');
  });
  const drawerCommentsList = drawerComments.length
    ? `<div class="review-drawer-comments">${drawerComments.slice(0, 8).map(comment => {
        ...
        return `<div class="review-drawer-comment-item ${resolved ? 'resolved' : ''}" data-review-drawer-comment="${esc(comment.id)}"><div class="review-drawer-comment-head"><strong>${esc(authorName)}</strong><span>${resolved ? '已处理' : '待处理'}</span></div><p>${esc(body.length > 60 ? body.slice(0, 60) + '…' : body)}</p></div>`;
      }).join('')}${drawerComments.length > 8 ? `<p class="review-drawer-more">还有 ${drawerComments.length - 8} 条意见，前往"处理审核意见"步骤查看完整列表。</p>` : ''}</div>`
