...
本文定义 `enterprise-profile/v1` 的逻辑结构、加载时序、配置指纹、脱敏投影和错误语义。机器契约以仓库根目录 `packages/platform-contracts/enterprise-profile/v1/enterprise-profile.schema.json` 为事实源，本文作为 TASK-KPG-03 双端加载器的开发输入，不定义凭证存储实现。
...
- Profile 由受控注册 ID 选择，文件引用以仓库根目录为基准。允许根包括能力包契约、领域、Skill 和已登记 Prompt 资源，不允许笼统引用含密钥或部署路径的 `config/knowledge-center/`；Schema 拒绝 URI、绝对路径、反斜杠、重复分隔符、控制字符、`.` 和 `..`，TASK-KPG-03 继续做存在性、普通文件和符号链接检查。
...
- `generationPolicies` 是可选的文档生产策略资源。文档类型与生产模式分离，策略统一约束来源确认、Agent/Skill/Prompt/模板、质量规则、逻辑输出目录和人工审核门禁。增量生成必须人工选源并确认；全量或大纲抽取可先产生候选，但必须确认并固化快照后才进入生成。
- 策略选择顺序为显式 `generationPolicyId`、手册/节点绑定、文档类型与模式唯一映射、能力包默认策略；缺失或冲突不得由模型猜测。任务需冻结策略 ID、版本和指纹，策略变更只影响新任务。
- 策略资源只引用能力包已登记资源 ID，不允许任意 URL、绝对路径、命令、目标仓库、分支、密钥或可执行代码；输出只能引用逻辑目录。
- 策略管理是独立的平台治理小模块，负责版本、启停、适用类型、组合资源、来源规则和质量门禁；文档生产只消费已发布策略。类型中文名可由策略声明的 `documentTypeNames` 或显示名称兼容匹配，运行时代码不维护企业类型到策略 ID 的硬编码映射。
