...
保证知识中心管理平台的四个运行单元能够由一个权威入口成组重启：文档 API、资料加工 API、认证 API 和 Web 网关。目录重构后不再保留多套含义重叠的启停脚本。

## 启动顺序

1. 检查 Node、Python、curl、前端依赖和所有应用目录。
2. 构建 `apps/pingcode-web`，构建失败则不改变现有运行进程。
3. 只停止状态目录中记录且工作目录、命令行均匹配的进程。
4. 确认目标端口均空闲，再依次启动文档 API、资料加工 API、认证 API 和 Web 网关。
5. 每个单元启动后轮询真实 HTTP 健康端点；任一单元失败则停止本轮已经启动的进程，避免部分可用状态。
6. 对外提供 `restart`（默认）、`status` 和 `stop` 三个动作。
...
脚本仍是单机进程编排，不替代 systemd、Docker Compose 或进程管理器。生产部署应将四个运行单元分别纳入进程监管与自动重启策略。
...
```bash
./tools/repository/restart-knowledge-center-isolated.sh
./tools/repository/restart-knowledge-center-isolated.sh status
./tools/repository/restart-knowledge-center-isolated.sh stop
```
