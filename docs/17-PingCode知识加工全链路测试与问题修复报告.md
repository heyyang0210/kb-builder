...
本专项验证下载任务状态不会被前端自行推断，加工预检与创建接口必须使用同一服务端门禁。

| 用例 | 前置下载快照 | 操作 | 期望结果 |
|---|---|---|---|
| TC-DG-01 | `completed` | 执行预检并创建加工任务 | 保持既有正常放行，无部分下载告警 |
| TC-DG-02 | `interrupted`、`canResume=true`、`completed=1`，且无其他活动任务 | 执行预检、确认开始加工并轮询任务 | 预检和启动均可通过；加工页持续显示“资料下载已中断，请先到‘下载文件’继续下载。本次将基于已下载资料加工。”；仅处理已完成资源，继续下载入口仍可用 |
| TC-DG-03 | `interrupted`、`canResume=false`、`completed>0` | 执行预检和创建加工任务 | 预检不可开始，创建返回 `BATCH_NOT_READY`，不创建加工运行目录或数据集 |
| TC-DG-04 | `interrupted`、`canResume=true`、`completed=0` | 执行预检和创建加工任务 | 预检不可开始，创建返回 `BATCH_NOT_READY` |
| TC-DG-05 | `queued/running/pausing/paused/cancelling/cancelled/failed` 任一状态 | 执行预检和创建加工任务 | 均保持阻断，返回与状态匹配的中文恢复建议 |

真实 API 验收需在隔离批次完成：读取下载任务快照、调用预检、创建加工任务、轮询终态并核对运行告警、处理资源数与下载完成数量一致；不得使用生产批次或历史产物替代本次结论。
