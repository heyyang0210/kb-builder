...
> 多角色工程化部署与使用说明：[Codex 多角色部署指南](docs/codex-multi-agent-deployment-guide.html)

> 提示词治理与认知复盘：[提示词与认知复盘记录](prompt.md)

> 仓库提交要求：[Git 提交规范](git提交规范.md)
...
仓库采用应用化 monorepo 分层：可部署服务位于 `apps/`，共享业务能力位于 `packages/`，运维与加工入口位于 `tools/`，测试按层级位于 `tests/`，日志、缓存和其他运行产物位于 `var/`。数据库连接的非敏感配置统一位于 `config/database/`，密码和 Token 仅通过环境变量或密钥系统注入。完整迁移映射见 [`docs/application-structure-refactor-checklist.md`](docs/application-structure-refactor-checklist.md)。

常用入口：

- 知识中心 API：`apps/knowledge-center-api/server.js`
- 知识中心 Web：`apps/knowledge-center-web/frontend-server.js`
- PingCode API：`apps/pingcode-api/app/main.py`
- PingCode Web：`apps/pingcode-web/`
- 运行数据：`var/agent-runner/`、`var/pingcode/`
...
配置保存在 `config/knowledge-center/model-config.json`，API Key 使用 AES-256 加密。
...
配置保存在 `config/knowledge-center/mcp-config.json`。
...
检查 `config/knowledge-center/mcp-config.json`：
- Server URL 是否正确
...
**相关文档**：
- 执行引擎设计文档：`packages/agent-runner-core/DESIGN.md`
- 前端设计文档：`prompt-generator-design.md`
- MCP 检索流程设计：`docs/agent-runner/19-知识点驱动的MCP检索流程设计.md`
- Retriever 调研（顶级公司方案）：`docs/agent-runner/22-Retriever阶段调研-顶级公司方案.md`
- Retriever 精准检索优化设计：`docs/agent-runner/23-Retriever精准检索优化设计.md`
