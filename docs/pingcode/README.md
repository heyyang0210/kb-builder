...
- [图片保真与统一预览设计](../../../docs/agent-runner/29-PingCode下载图片保真与统一文档预览设计.md) - 页面图片下载及预览接口
- [文件分页与加工配置设计](../../../docs/agent-runner/30-PingCode文件清单分页与加工配置设计.md) - 资源分类、分页和清洗预设
- [资料加工任务来源识别与筛选设计](../../../docs/agent-runner/35-素材批次来源识别与筛选界面设计.md) - 来源、归属、筛选、聚合和服务端分页
- [资料加工平台工作台重构设计](../../../docs/agent-runner/36-资料加工平台工作台与全流程前端重构设计.md) - 工作台、任务聚合接口、详情抽屉和全流程术语
...
# 集成测试
python3 tests/pingcode/test_integration.py
```

## 素材平台前端

当前非容器化实现位于 `tools/pingcode-cli/web/`，前端和后端地址均通过运行时环境配置，不依赖固定机器路径。

### 启动后端

```bash
python3 -m pip install --user --break-system-packages -r apps/pingcode-api/requirements.txt
