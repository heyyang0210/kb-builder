Created by 易文亮, last modified on 十一月 07, 2023
...
IR链接：    [YDBRD-20532](https://jira.yasdb.com/browse/YDBRD-20532?src=confmacro)    -  LSC表增量同步性能优化  完成

##   [1. 需求概述](https://conf.yasdb.com/pages/viewpage.action?pageId=133568047#1-%E9%9C%80%E6%B1%82%E6%A6%82%E8%BF%B0)  

T+N数据同步能力，需要优化LSC的同步性能；以lineitem表的性能持平starrocks作为牵引目标，而热数据导入性能差，需要转冷，考虑实现冷数据增量导入方案，要求去重。
...
使用hint语法，insert into带 /* bulkload */ ，结合适配datax同步工具。使用批插直接写入冷数据，根据主键/唯一键去重。写入的冷数据是未排序的，如果有重复记录，会打上delete标志，带转冷后走合并排序流程进行处理。

增量导入前要求事务完结，会创建一个savepoint，增量导入过程中不允许做其他dml操作，commit后数据才可见。
...
*主要使用等价类结合场景测试法，主要功能用例维护到yasft，部分KT+一致性+HA等专项用例维护到各对应CI工程*
