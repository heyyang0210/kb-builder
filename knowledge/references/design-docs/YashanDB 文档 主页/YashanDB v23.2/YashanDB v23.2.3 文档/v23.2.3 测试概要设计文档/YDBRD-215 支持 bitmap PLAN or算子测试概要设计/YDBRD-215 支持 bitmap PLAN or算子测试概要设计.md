Created by 马文英, last modified on 十二月 11, 2023

##   [1. 需求概述](https://conf.yasdb.com/pages/viewpage.action?pageId=133568047#1-%E9%9C%80%E6%B1%82%E6%A6%82%E8%BF%B0)  

  [YDBRD-215](https://jira.yasdb.com/browse/YDBRD-215?src=confmacro)    -  支持bitmap PLAN OR算子  待RMT评审
...
查询语句中，查询条件分别属于多个索引时，将btree索引转换为bitmap索引，通过bitmap and/or提升检索效率
...
1、可测性：需要提供类似算子cost的配置，当配置该参数时，可以使优化器选择 bitmap plan or算子

2、特性开关：必要时可以关闭这个算子。oracle提供了

3、bitmap plan or 可以使用索引的个数

4、优化器是否能正确选择 bitmap plan or。 bitmap plan or和 非bitmap计划性能对比

5、outline固定执行计划

6、DML是否会走bitmap plan or

7、bitmap的大小，bitmap的内存从那块分配
