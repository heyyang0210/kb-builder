Created by 史鑫, last modified on 九月 27, 2024
...
- **validate_password_dictionary_file**  ：黑名单
    - 文件大小：最大支持1M(#define MAX_DICTIONARY_FILE_LENGTH 1024 * 1024)
    - 单个名单长度：仅看100字节，大于100报错
    - 路径：非绝对路径，concat yasdb_data
    - 每次set。重新加载
        - 并发控制：
            - s  et：load file latch X (mysql:read_dictionary_file)
            - check：read latch s (mysql:validate_dictionary_check)
    - 校验规则：密码中  **4个及以**  上任意的连续字符，不能在黑名单内 
    - 大小写：sql语句中密码全部转成  **小写**  去对比；
    - 字符集：sql语句中的密码全部转成  **utf8**  ，求文件对比。即：认为文件中的都是utf8格式
- 工作量：开发/自测2days，测试2days
...
select @@global.validate_password_dictionary_file;

begin
	execute immediate 'set @@global.validate_password_dictionary_file="D:/anchorhome/mysqlcert/a"';
	end;
/


begin
	execute immediate 'set @@global.validate_password_policy=2';
	end;
/

drop user user1;
create user user1 identified by 'Cod-2022';
drop user user1;
create user user1 identified by 'xCod-2022';
create user user1 identified by 'xxxxxxCod-2022xxxxxxx';
create user user1 identified by 'xxxxxxabcde-23Cxxxxxxx';
```
...
[自测用例.xls](https://pingcode.yasdb.com/atlas/file/origin-url?version=undefined&action=download&token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..wzO_e8W73xNjxC50D7ZJwL4kbhvkzCOSYKwEgLK3WNg)
...
|  [](null)  ,生效时机,Posted by shixin at 七月 05, 2024 10:43|
|---|
