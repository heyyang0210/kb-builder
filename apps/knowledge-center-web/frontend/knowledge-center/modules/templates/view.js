...
export function renderTemplates({ templateProjection = {}, templateUi = {} }) {
  ...
  return `<div class="page-intro"><div><h2>模板管理</h2><p class="muted">维护模板内容，历史版本随保存自动保留。</p></div>${editable ? '<div class="template-toolbar"><button class="button secondary" type="button" data-template-new>新建模板</button><input type="file" data-template-file accept=".md,.markdown" multiple hidden></div>' : ''}</div>
    ...
    ${editable ? '<div class="template-dropzone" data-template-dropzone tabindex="0" role="button" aria-label="上传 Markdown 模板：拖拽文件到此处，或点击选择文件">上传 Markdown 模板：拖拽文件到此处，或点击选择文件（支持多选，单个不超过 10MB）</div>' : ''}
    ...
    <div class="template-workspace" aria-busy="${Boolean(ui.busy)}"><aside class="template-list" aria-label="模板列表">
      ...
      <div data-template-results>${renderTemplateResults({ templateProjection, templateUi })}</div>
    </aside><section class="template-editor" aria-label="模板内容"><header><div><h3>${escapeHtml(display?.name || '选择模板')}</h3><p class="muted">${display ? `${statusName(selected && !isNew ? selected.status : '')} · ${selected && !isNew ? `当前版本 v${selected.currentVersion}` : '保存后创建首个版本'}` : '从左侧选择模板，或上传文档创建草稿。'}</p></div>${selected && !isNew ? `<button class="icon-button" type="button" data-template-history aria-expanded="${Boolean(ui.historyOpen)}" aria-controls="template-history-panel" aria-label="查看历史版本" title="历史版本"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M3 11a9 9 0 1 1 2 7M3 4v7h7M12 7v5l3 2"/></svg></button>` : ''}</header>
      ${display ? `${historical ? `<div class="template-history-banner">正在只读查看历史版本 v${historical.version}<button class="button secondary" data-template-current>返回当前内容</button></div>` : ''}
        ${editable && !historical ? `<div class="template-metadata"><label>模板名称<input data-template-field="name" value="${escapeHtml(draft?.name ?? selected?.name ?? '')}" required></label><label>类型<input data-template-field="type" value="${escapeHtml(draft?.type ?? selected?.type ?? '')}" required></label><label class="template-description">描述<input data-template-field="description" value="${escapeHtml(draft?.description ?? selected?.description ?? '')}" required></label></div>` : `<p>${escapeHtml(display.description || '')}</p>`}
        <div class="template-toolbar" aria-label="内容显示方式"><button class="button secondary" data-template-mode="preview" aria-pressed="${mode === 'preview'}">预览</button>${editable && !historical ? `<button class="button secondary" data-template-mode="edit" aria-pressed="${mode === 'edit'}">Markdown 编辑</button>` : ''}${draft?.dirty ? '<span class="muted">有未保存草稿</span>' : ''}</div>
        ${mode === 'edit' && editable ? `<label class="template-content-label">Markdown 正文<textarea data-template-content spellcheck="false">${escapeHtml(content)}</textarea></label>` : `<article class="template-preview" aria-label="模板预览">${renderMarkdown(content)}</article>`}
        <footer class="template-actions"><span class="muted">${historical ? `历史版本 v${historical.version} · ${date(historical.createdAt)}` : selected && !isNew ? `${selected.current?.validationStatus === 'passed' ? '校验通过' : '待校验'}` : '新模板尚未保存'}</span><div>${manageable && selected && !isNew && !historical ? `<button class="button secondary" data-template-toggle ${ui.busy ? 'disabled' : ''}>${selected.status === 'active' ? '停用' : '恢复启用'}</button>` : ''}${editable && !historical ? `${isNew ? '<button class="button secondary" data-template-cancel>取消</button>' : ''}<button class="button" data-template-save ${ui.busy ? 'disabled' : ''}>${ui.busy ? '处理中…' : '保存'}</button>` : ''}</div></footer>` : ''}
