...
class TrainingService:
    def __init__(
        ...
        *,
        ...
        self.graph_observability_service = GraphObservabilityService(
            self, Path(__file__).resolve().parents[3] / "config" / "pingcode" / "graph-observability-rules.json"
        )
        ...
        default_skill_root = Path(__file__).resolve().parents[3] / "tools" / "knowledge-processing" / "pingcode-processing" / "skills"
    ...
    def _require_batch_ready(self, batch_id: str):
        ...
        for task in tasks:
            if (
                ...
                and task.id not in resumable_download_ids
                and not self._is_completed_cancelled_download(task)
            ):
    ...
    def _is_resumable_partial_download(task: TaskSnapshot) -> bool:
        ...
        return bool(
            task.type == "download"
            and task.state in {"interrupted", "cancelled"}
            and task.can_resume
            and task.completed > 0
        )

    @staticmethod
    def _is_completed_cancelled_download(task: TaskSnapshot) -> bool:
        """取消在所有页面完成后到达时，任务状态仍为 cancelled 但批次可直接加工。"""
        detail = task.progress_detail or {}
        return bool(
            task.type == "download"
            and task.state == "cancelled"
            and "pendingPages" in detail
            and int(detail.get("pendingPages", 0) or 0) == 0
            and task.failed == 0
            and (task.total is None or task.completed >= task.total)
        )
    ...
    def _keyword_filter_resource_paths() -> tuple[Path, Path]:
        ...
        return (
            repo_root / "knowledge/skills" / "keyword-filter" / "SKILL.md",
    ...
    def _keyword_filter_prompt() -> tuple[str, str]:
        repo_root = Path(__file__).resolve().parents[3]
        skill_path = repo_root / "knowledge/skills" / "keyword-filter" / "SKILL.md"
    ...
    def _extract_deterministic(
        ...
        try:
            ...
            for collection_name, kind in candidate_collections:
                for item in aggregated.get(collection_name, []):
                    chunk_id = item.get("chunkId", "")
                    chunk = next((c for c in chunks if c.get("id") == chunk_id), None)
                    if chunk:
                        candidates.append(self._model_knowledge_candidate(
                            task_id, kind, item, chunk,
            ...
            self._log(
                ...
                (
                    "Workflow Agent 知识提取完成："
                    f"{metadata.get('knowledge_point_count', 0)} 个知识点、"
                    f"{metadata.get('entity_count', 0)} 个实体、"
                    f"{metadata.get('relation_count', 0)} 个关系"
                ),
                details={**metadata},
            )
        except Exception as error:
        ...
        self._set_stage(
            ...
            unit="个处理单元",
            detail_message=(
                f"Workflow Agent 知识提取完成：{len(candidates)} 个知识候选，{len(human_required)} 个质量问题"
            ),
            model_calls=dict(calls),
        )
