...
class TaskEventBroker:
    ...
    def stream(self, task_id: str, last_event_id: int = 0):
        ...
        # 立即发送连接确认，避免无待处理事件时首字节被 15 秒轮询间隔阻塞。
        # 这是一条 SSE 注释，不改变既有业务事件顺序和重放游标语义。
        yield ": connected\n\n"
        while True:
            with self._condition:
...
class TaskService:
    ...
    def create_download(self, batch_id: str, idempotency_key: str | None) -> TaskSnapshot:
        ...
        task = TaskSnapshot(
            ...
            # 取消只在页面边界生效，队列态也允许先登记取消信号，避免线程启动竞态。
            can_cancel=True,
            created_at=now,
            updated_at=now,
        )
    ...
    def retry(self, task_id: str) -> TaskSnapshot:
        ...
        updated = current.model_copy(
            update={"state": "queued", "message": None, "can_cancel": True, "can_retry": False, "can_resume": False, "updated_at": utcnow()}
        )
        self._save(updated)
    ...
    def resume(self, task_id: str) -> TaskSnapshot:
        current = self.get(task_id)
        if current.state not in {"interrupted", "paused", "failed", "cancelled"}:
            return current
        updated = current.model_copy(
            update={"state": "queued", "message": None, "can_cancel": True, "can_retry": False, "can_resume": False, "updated_at": utcnow()}
        )
    ...
    def cancel(self, task_id: str) -> TaskSnapshot:
        current = self.get(task_id)
        if current.state not in {"queued", "running"} or not current.can_cancel:
            return current
        event = self._cancel_events.setdefault(task_id, threading.Event())
        event.set()
        return self._update_download_snapshot(
            task_id,
            state="cancelling",
            stage="cancelling",
            can_cancel=False,
    ...
    def _run_download(self, task_id: str, mode: str = "all") -> None:
        ...
        current = self.get(task_id)
        batch = self.batches.get(current.batch_id)
        if cancel_event.is_set() or current.state == "cancelling":
            self._mark_download_cancelled(task_id, batch)
    ...
    def _mark_download_cancelled(self, task_id: str, batch: MaterialBatch) -> TaskSnapshot:
        task = self.get(task_id)
        state_dir = self._download_state_dir(batch)
        page_state = self._read_page_state(state_dir / "pages.jsonl")
        total = task.total or batch.source_snapshot.estimated_pages
        summary = self._download_summary(page_state, total, 0)
        resumable = summary["progressDetail"]["pendingPages"] > 0 or summary["failed"] > 0
        updated = self._update(
            ...
            message="下载已取消，可继续未完成页面。" if resumable else "下载已取消。",
        )
        self.events.publish_event(
            task_id,
        ...
        # 取消是终态任务，不应再占用批次的活动任务列表。重新读取批次以保留取消过程中其他任务新增的 ID。
        try:
            latest_batch = self.batches.get(batch.id)
        except KeyError:
            latest_batch = batch
        task_by_id = {item.id: item for item in self.list(batch.id)}
        active_ids = [
            item
            for item in latest_batch.active_task_ids
            if item != task_id
            and task_by_id.get(item) is not None
            and task_by_id[item].state in {"queued", "running", "cancelling"}
        ]
        batch_state = "downloading" if resumable or active_ids else "downloaded"
        self.batches.update(batch.id, state=batch_state, activeTaskIds=active_ids)
        return updated
