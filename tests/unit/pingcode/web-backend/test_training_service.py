...
from app.agents.knowledge_extraction_agent import KnowledgeExtractionWorkflowAgent
...
class DeterministicPipelineTests(unittest.TestCase):
    ...
    def test_training_allows_resumable_cancelled_download_with_completed_materials(self):
        now = utcnow()
        batch = SimpleNamespace(id="batch-cancelled", state="downloading", active_task_ids=[])
        download_task = TaskSnapshot(
            id="task-download",
            ...
            failed=0,
            canResume=True,
            progressDetail={"pendingPages": 1},
            createdAt=now,
            updatedAt=now,
        )
        service = TrainingService(None, FakeBatches(batch), FakeTasks(download_task), None, None, gateway=None)
        self.assertIs(service._require_batch_ready("batch-cancelled"), batch)

    def test_training_allows_cancelled_download_when_all_pages_complete(self):
        now = utcnow()
        batch = SimpleNamespace(id="batch-cancelled-complete", state="downloaded", active_task_ids=[])
        download_task = TaskSnapshot(
            id="task-download",
            batchId="batch-cancelled-complete",
            type="download",
            state="cancelled",
            stage="cancelled",
            completed=2,
            ...
            failed=0,
            canResume=False,
            progressDetail={"pendingPages": 0},
            createdAt=now,
            updatedAt=now,
        )
        service = TrainingService(None, FakeBatches(batch), FakeTasks(download_task), None, None, gateway=None)
        self.assertIs(service._require_batch_ready("batch-cancelled-complete"), batch)
    ...
    def test_workflow_aggregation_preserves_entities_relations_and_trace(self):
        agent = object.__new__(KnowledgeExtractionWorkflowAgent)
        aggregated = agent._result_aggregation({
            "verified_results": [{
                "chunkId": "resource-1:0",
                "agentTaskId": "agent-1",
                "modelCallId": "call-1",
                "knowledgePoints": [{"title": "连接限制", "evidenceText": "p_size"}],
                "entities": [{"name": "p_size", "evidenceText": "p_size"}],
                "relations": [{
                    "source": "p_size",
                    "target": "连接数",
                    "type": "AFFECTS",
                    "evidenceText": "p_size 影响连接数",
                }],
            }],
            "rejected_items": [],
        })["final_knowledge"]

        self.assertEqual(len(aggregated["knowledgePoints"]), 1)
        self.assertEqual(len(aggregated["entities"]), 1)
        self.assertEqual(len(aggregated["relations"]), 1)
        self.assertEqual(aggregated["entities"][0]["agentTaskId"], "agent-1")
        self.assertEqual(aggregated["relations"][0]["modelCallId"], "call-1")
        self.assertEqual(aggregated["metadata"]["total_items"], 3)
