...
class DownloadResumeTests(unittest.TestCase):
    ...
    def test_cancel_marks_download_cancelled_and_resumable(self):
        self.tasks._update("task-download", state="running", can_cancel=True)

        cancelling = self.tasks.cancel("task-download")
        self.assertEqual(cancelling.state, "cancelling")

        # 线程在页面边界检查取消信号；直接运行 worker 可确定性覆盖该边界。
        self.tasks._run_download("task-download", "resume")

        cancelled = self.tasks.get("task-download")
        self.assertEqual(cancelled.state, "cancelled")
        self.assertTrue(cancelled.can_resume)
        self.assertIn("已取消", cancelled.message)
        batch = self.tasks.batches.get(self.batch.id)
        self.assertEqual(batch.state, "downloading")
        self.assertEqual(batch.active_task_ids, [])
        listed = self.tasks.batches.list_page(page=1, page_size=20, has_active_task=False)
        self.assertEqual(listed["total"], 1)

    def test_queued_cancel_removes_task_from_active_batch(self):
        state_dir = self.data_root / "spaces" / "yasdoc" / "batches" / self.batch.id / "download-state"
        (state_dir / "pages.jsonl").write_text("", encoding="utf-8")
        self.tasks._update(
            "task-download",
            state="queued",
            completed=0,
        ...
        self.tasks.cancel("task-download")
        self.tasks._run_download("task-download", "all")
    ...
    def test_cancel_after_all_pages_complete_marks_batch_downloaded(self):
        state_dir = self.data_root / "spaces" / "yasdoc" / "batches" / self.batch.id / "download-state"
        (state_dir / "pages.jsonl").write_text(
            "\n".join([
                '{"taskId":"task-download","pageId":"page-1","pageName":"页面一","state":"completed","warningCount":0}',
                '{"taskId":"task-download","pageId":"page-2","pageName":"页面二","state":"completed","warningCount":0}',
            ]),
            encoding="utf-8",
        )
        self.tasks._update("task-download", state="running", completed=2, total=2, can_cancel=True)
    ...
    def test_cancel_preserves_other_active_tasks(self):
        self.tasks.batches.update(self.batch.id, activeTaskIds=["task-download", "task-other"])
        self.store.put_record("tasks", "task-other", TaskSnapshot(
            id="task-other",
            batch_id=self.batch.id,
            type="download",
            state="running",
        ...
        batch = self.tasks.batches.get(self.batch.id)
        self.assertEqual(batch.active_task_ids, ["task-other"])
        self.assertEqual(batch.state, "downloading")
...
class SkillRegistryTests(unittest.TestCase):
    def test_repository_contains_new_skill_architecture(self):
        ...
        root = Path(__file__).resolve().parents[4] / "tools" / "knowledge-processing" / "pingcode-processing" / "skills"
