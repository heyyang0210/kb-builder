...
1.delete本地表filter子查询带远端表，支持，支持依赖于当前dblink select的支持范围。

2.实现delete远端表， 多表delete拦截报错。

SR链接：    [YDBRD-13335](https://jira.yasdb.com/browse/YDBRD-13335?src=confmacro)    -  实现DBLINK的DML DELETE单表能力  完成
...
1. delete远端表，对delete的filter有如下限制（与dblink 单表update filter限制一样）：
1. *投影列，aggr聚集函数，窗口函数，子查询，sequence，udf在filter中出现当前会拦截报错
1. delete远端表， filter中使用内置函数，按照yasdb内部函数进行校验，如yasdb内部没有实现的函数，校验报错。
1. 多表delete报错。
...
对于单表delete， yasdb将完整的sql语句发送到yex_server,  yex_server调用odbc接口发送到oracle执行，并将oracle返回的执行结果返回给yasdb。
...
CodResult verifyDblinkDelete(AnlVerifier* vrfr, DeleteContext* context);

CodResult execDblinkDelete(AnlStmt* stmt)；
...
2.delete本地表 + filter 子查询里面有远端表查询(select * from 远端表，没有复杂子查询用例)。

3.delete远端表+普通filter，以及filter里面有出现限制需要拦截报错用例。

4.多表delete拦截报错

5. expr里面使用函数：verify阶段会校验该函数是否合法，也就是如果yasdb没有实现的但是oracle有的函数，在dblink中使用是会报错的

6.存储过程使用dblink 单表delete， 包括有绑定参数的情况
